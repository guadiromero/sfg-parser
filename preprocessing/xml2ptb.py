       ...
       # pop the last element in the stack
        ...
        if node == ')':
            ...
            if node.tag == "Constituent" and ellipsed_node == True:
                # for the right strategy, keep this information to add it to the immediate next node
                right_label = constituent_id
                ...
                for previous_node in node_metadata:
                    ...
                    if node_type == "Ellipsis":
                        if previous_node == node.find("ConstituentRef").get("idref"):
                            ...
                            right_label = node_metadata[previous_node]["constituent_id"]
    ...
    for node in node_metadata:
        ...
        if node_metadata[node]["terminal"] == "no":
            if ellipsis_method == "left_label" and node_metadata[node]["left_label"] > 0:
                    ...
                    new_label = node_metadata[node]["tag"] + "ellipsis" + str(node_metadata[node]["right_label"])
            else:
        ...
        # for terminals
            ...
            if ellipsis_method == "left_label" and node_metadata[node]["left_label"] > 0:
                    ...
                    new_label = node_metadata[node]["pos"] + "ellipsis" + str(node_metadata[node]["right_label"]) + " " + node_metadata[node]["text"]
            else:
